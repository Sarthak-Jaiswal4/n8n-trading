...
app.put("/workflow/:workflowid", middleware, async (req, res) => {
    ...
    try {
        ...
        const workflow = await WorkflowModel.findByIdAndUpdate(workflowID, {
...
app.post('/fyers/getToken', middleware, async (req, res) => {
    ...
    const fyers = new Fyers(App_ID, Redirect_URL, Secret_ID);
    const url = fyers.GenerateURL();
    const token = await fyers.getAccesstoken(Auth_Code, Secret_ID);
    if (token.code == 200) {
        const updatecred = await FyersModel.findOneAndUpdate({ UserID: new mongoose.Types.ObjectId(userID) }, {
...
app.post('/fyers/Placeorder', middleware, async (req, res) => {
    ...
    try {
        ...
        fyers.place_order(data).then((response) => {
...
async function executor(workflow, trigger) {
